...
const state = {
  items: [
    {id:'A', name:'Pedang Pangu', lvl:0},
    {id:'B', name:'Zirah Naga', lvl:0},
    {id:'C', name:'Cincin Dewa', lvl:0},
  ],
  onTable: null,
...
document.getElementById('reset-all').onclick = () => {
  ...
  updateCostsUI();
  updateUsageUI();
  $slot.innerHTML = '<span class="slot-text">Seret Equipment Ke Sini</span>';
  $log.textContent = '';
};
...
function placeOnTable(id){
  ...
  state.onTable = id;
  $slot.innerHTML = `
    <strong style="color:var(--gold-light); font-size:1.2rem; display:block; margin-bottom:0.8rem">${it.name}</strong> 
    <span class="chip lvl" style="font-size:1rem; padding: 0.3rem 0.8rem">Level Saat Ini: +${it.lvl}</span>
  `;
}
...
async function refineOnce(){
  if (!state.onTable){ log('Seret equipment ke Altar Refine!', 'bad'); return; }
  const it = state.items.find(x=>x.id===state.onTable);
  if (it.lvl >= MAX_LEVEL){ log(`Equipment telah mencapai level maksimum (+${MAX_LEVEL}).`, 'ok'); return; }
  ...
  if (res.success) {
    ...
    else if (res.applied_rule === 'stay')  msg += ' Level tidak berubah.';
    else if (res.applied_rule === 'drop')  msg += ' Level turun 1 tingkat.';
    log(msg, 'bad');
  }
}
